  ...
  socket.on('dregljaj', function (dregljaj) {
    if (dregljaj.vzdevek) {
      ...
      if (socketIdNaslovnika && socketIdNaslovnika !== socket.id) {
       io.sockets.sockets[socketIdNaslovnika].emit('dregljaj', {dregljaj: true, vzdevek: false});
       io.sockets.sockets[socket.id].emit('dregljaj', {dregljaj: false, vzdevek: dregljaj.vzdevek});
      } else {
       io.sockets.sockets[socket.id].emit('dregljaj', {dregljaj: false, vzdevek: false});
      }
    }
  });
}
